...
import { Cpu, History, Eye, EyeOff } from 'lucide-react';
...
export const TopBar: React.FC<TopBarProps> = React.memo(({ isSessionHistoryOpen, onToggleSessionHistory }) => {
  const { state, undoCount, redoCount } = useInventory();
  ...
  const undoText = undoCount === 1 ? '1 undo action' : `${undoCount} undo actions`;
  const redoText = redoCount === 1 ? '1 redo action' : `${redoCount} redo actions`;
  const accessibleLabel = `Session history: ${undoText} and ${redoText}.`;

  return (
    <header className="sticky top-0 z-50 bg-[#0D1118]/95 backdrop-blur-md border-b border-white/[0.08] px-2.5 sm:px-4 py-2 mb-2 flex flex-row items-center justify-between gap-2 shadow-sm min-h-[48px]">
      {/* Title */}
      ...
      {/* Right side: Financial Summary Pills + Mobile History */}
      <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
        ...
        <button
          ...
          className={`w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-xl transition-colors border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7C6CF2] cursor-pointer shrink-0 ${
            hideSupplierNames
        ...
        <div className="md:hidden flex items-center relative">
          <button
            ...
            aria-controls="session-history-popover-mobile"
          ...
          >
            <div className="relative flex items-center justify-center">
              <History className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#7C6CF2]" />
              {totalHistoryCount > 0 && (
                <span className="absolute -bottom-1.5 -right-1.5 text-[8px] sm:text-[9px] font-mono font-bold text-[#9D91FA] bg-[#0D1118] px-0.5 sm:px-1 rounded-full border border-white/[0.08]">
                  {totalHistoryCount}
                </span>
              )}
            </div>
          ...
          {isSessionHistoryOpen && (
            <div className="absolute right-0 top-full mt-2 w-[calc(100vw-1.5rem)] max-w-[360px] z-[300]">
              <SessionHistoryPopover
                id="session-history-popover-mobile"
